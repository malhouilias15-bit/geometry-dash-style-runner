...
function spawnNormal() {
  ...
  // DOUBLE WALL + HIGHER PLATFORM
  if (Math.random() < 0.4) {
    walls.push({ x: x + 40, w: 30, h: 60, passed: false });

    platforms.push({
      x: x - 10,
      y: canvas.height - groundH - 95, // higher than before
      w: 130,
      h: 10
    });
  }
...
function spawnHard() {
  ...
  // BIG WALL + ORB INSIDE WALL
  if (Math.random() < 0.4) {
    walls.push({ x, w: 70, h: 150, passed: false });

    // orb in middle of wall (like your drawing)
    orbs.push({
      x: x + 35,
      y: canvas.height - groundH - 75
    });
...
function update() {
  ...
  scoreEl.textContent = `Score: ${score} | $${money} | Level: ${level}`;
}
...
function renderNotepad() {
  noteMenu.innerHTML = `
    <b>OWNER PAD</b><br><br>
    <input id="giveUser" placeholder="type username" style="width:140px"><br><br>
