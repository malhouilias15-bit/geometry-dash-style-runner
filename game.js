...
let unlockedSkins = JSON.parse(localStorage.getItem("unlockedSkins")) || { green: true };
let snakeSkin = unlockedSkins.selected || "green";
...
function spawnWalls() {
  ...
  walls.push({ x, y, w: 26, h: 60, scored: false, main: true });

  if (demonMode) {
    walls.push({ x: x + 40, y, w: 26, h: 60, scored: true, main: false });
...
function update() {
  ...
  walls.forEach(w => {
    ...
    if (w.main && !w.scored && w.x + w.w < snake.x) {
      w.scored = true;
  ...
  platforms.forEach(p => {
    const wasAbove = snake.prevY + snake.r <= p.y;
    const falling = snake.vy >= 0;
