...
/* ================= NOTEPAD ================= */
let notepadBtn, notepadMenu;

if (username === OWNER_USERNAME) {
  notepadBtn = makeBtn("NOTEPAD", null, "20px", "20px");

  notepadMenu = document.createElement("div");
  Object.assign(notepadMenu.style, {
    position: "fixed",
    bottom: "70px",
    right: "20px",
    background: "#111",
  ...
  notepadMenu.innerHTML = `
<b>Give skin to</b><br>
<input id="noteInput" placeholder="darkred username"><br><br>
<button id="noteApply">Apply</button>
`;

  document.body.appendChild(notepadMenu);

  notepadBtn.onclick = () => {
    notepadMenu.style.display =
      notepadMenu.style.display === "none" ? "block" : "none";
  };

  document.getElementById("noteApply").onclick = () => {
    const [skin, user] = document.getElementById("noteInput").value.trim().split(" ");
    if (!skin || !user) return alert("Format: darkred username");
    forcedSkins[user] = skin;
    localStorage.setItem("forcedSkins", JSON.stringify(forcedSkins));
    alert(`Gave ${skin} to ${user}`);
  };
}
...
function spawnWalls() {
  ...
  if (demonMode) {
    ...
    platforms.push({ x: x - 10, y: y - 30, w: 120, h: 12 });
  }
}
...
function update() {
  ...
  walls.forEach(w => {
    ...
    if (!w.scored && w.x + w.w < snake.x) {
      w.scored = true;
      const gain = scoreBoostTimer > 0 ? 2 : 1;
      score += gain;
      money += gain;
    }
  });
