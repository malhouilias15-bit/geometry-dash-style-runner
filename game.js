...
function renderSkins() {
  const skins = ["green","blue","purple","yellow","orange","rainbow","darkred"];
  skinsMenu.innerHTML = skins.map(s => {
    if (!unlockedSkins[s]) return `<div style="opacity:.4">${s} (locked)</div>`;
...
function update() {
  ...
  walls.forEach(w => {
    if (
      !snake.onPlatform && // ✅ FIX: platform-safe
      snake.x + snake.r > w.x &&
