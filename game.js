...
function spawnHard() {
  ...
  if (Math.random() < 0.4) {
    ...
    // ORB TO SAFE PLATFORM
    orbs.push({ x: x + 10, y: canvas.height - groundH - 95, power: 18 });
...
function draw() {
  ...
  ctx.fillStyle = data.selected === "rainbow"
...
function skinBtn(c, req) {
  ...
  if (score >= req && c !== "rainbow") {
    data.skins[c] = true; save();
    return `<button onclick="equip('${c}')">EQUIP ${c}</button><br>`;
  }
  if (c === "rainbow" && score >= 100 && !hardest) {
    data.skins.rainbow = true; save();
    return `<button onclick="equip('rainbow')">EQUIP rainbow</button><br>`;
  }
