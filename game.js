...
function spawnHard() {
  ...
  // orbs BEFORE wall
  orbs.push({ x: x - 60, y: canvas.height - groundH - 120 });
  orbs.push({ x: x - 30, y: canvas.height - groundH - 150 });

  walls.push({ x, w: 60, h: 140 });

  for (let i = 0; i < 6; i++) {
    spikes.push({ x: x + i * 22, y: canvas.height - groundH, s: 20 });
  }

  platforms.push({ x: x + 10, y: canvas.height - groundH - 170, w: 50, h: 10 });
  platforms.push({ x: x + 70, y: canvas.height - groundH - 200, w: 50, h: 10 });
}
...
function update() {
  ...
  spikes.forEach(s => {
    if (player.x > s.x && player.x < s.x + s.s && player.y + player.r > s.y)
      gameOver = true;
  });
...
function unlockByScore() {
  ...
  if (score >= 40) data.skins.orange = true;
...
function renderSkins() {
  skinsMenu.innerHTML = Object.keys(data.skins).map(k => {
    return data.skins[k]
      ? `<button onclick="equip('${k}')">EQUIP ${k.toUpperCase()}</button>`
      : `${k.toUpperCase()} — LOCKED`;
  }).join("<br>");
}
...
function saveScore() {
  if (!data.leaderboard.find(e => e.name === username)) {
