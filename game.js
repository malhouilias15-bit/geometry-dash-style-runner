...
makeBtn("HARDEST LEVEL", "10px").onclick = () => {
  demonMode = !demonMode;
  resetGame();
};
...
function spawnBossWall() {
  ...
  escapeBlocks.push({ x: x + 10, y: y + 90, w: 50, h: 12 });
...
function update() {
  ...
  if (++wallTimer > 90) spawnWalls(), wallTimer = 0;
  if (++spikeTimer > 180) spawnSpikes(), spikeTimer = 0;
  if (demonMode && ++bossTimer > 700) spawnBossWall(), bossTimer = 0;
...
/* ================= DRAW ================= */
function draw() {
  ctx.fillStyle = demonMode ? "#550000" : "#000";
  ...
  ctx.fillStyle = unlockedSkins.selected === "rainbow"
    ? `hsl(${Date.now()%360},100%,50%)`
    : unlockedSkins.selected;
