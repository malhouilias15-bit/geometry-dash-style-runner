...
function spawnNormal() {
  ...
  for (let i = 0; i < spikeCount; i++) {
    spikes.push({ x: sx + i * 26, y: canvas.height - groundH, s: 22 });
  }
}
...
function spawnHard() {
  ...
  // BIG WALL
  walls.push({ x, w: 50, h: 140 });

  // ORB BEFORE WALL
  orbs.push({ x: x - 30, y: canvas.height - groundH - 90 });

  // 2 SAFE PLATFORMS
  platforms.push({ x: x + 5, y: canvas.height - groundH - 120, w: 60, h: 12 });
  platforms.push({ x: x + 70, y: canvas.height - groundH - 160, w: 60, h: 12 });
...
/* ================= SHOP ================= */
function renderShop() {
  shopMenu.innerHTML = `Money: $${money}`;
}
